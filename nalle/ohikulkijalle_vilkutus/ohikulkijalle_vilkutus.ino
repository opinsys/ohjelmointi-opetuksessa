...
#include <Servo.h> // Sisällytä ohjelmaan kirjasto Servo.h 
...
void setup() // Tehdään asetukset servolle ja lasketaan 2 sekunnin aikana valaistuksen kynnystä, jos se on oletusarvoa matalampi.
{
...
void loop() // Toista ikuisesti:
{
  ...
  if (valovastuksen_arvo < matalin_valovastuksen_arvo) { // Jos valovastuksen muuttuja on pienempi kuin kynnyksen muuttuja,
    ...
    for(int i = 1; i <= 2; i++) { // Suorita kunnes muuttujan i (alkuarvo=1) arvo on pienempi tai yhtäsuuri kuin 2. Silmukan lopussa nosta i:n arvoa yhdellä.
      tassu.write(120); // Käännä tassu asentoon 120 astetta ja 
      delay(150); // odota 150 millisekuntia.

      tassu.write(160); // Käännä tassu asentoon 160 astetta ja 
      delay(150); // odota 150 millisekuntia.
    } // Nostetaan i:n arvoa yhdellä ja palataan for-silmukan alkuun, jos sen ehto on edelleen totta.
